...
class GameLogicService {
  ...
  async submitGuess(sessionId: string, guess: string): Promise<GameResult> {
    ...
    const isCorrect = guess.toLowerCase() === session.correctAnswer.toLowerCase()
  ...
  async getNextPokemon(sessionId: string, config: GameConfig): Promise<GameSession> {
  ...
  private calculateScore(
    ...
    const { baseScore, timeBonusMultiplier, streakMultiplier, difficultyMultiplier, rarityMultiplier } = this.scoringConfig

    // Base score
    let score = baseScore

    // Time bonus (faster = higher bonus)
    const timeBonus = Math.max(0, (30 - timeTaken) * timeBonusMultiplier)
    score += timeBonus

    // Streak multiplier
    const streakBonus = Math.min(streak * streakMultiplier, 2.0) // Cap at 2x
    score *= (1 + streakBonus)
  ...
  calculateGameStats(sessions: GameSession[]): {
    ...
    return {
      ...
      averageTime: completedSessions.length > 0 
        ? completedSessions.reduce((sum, s) => sum + ((Date.now() - s.startTime) / 1000), 0) / completedSessions.length
        : 0,
      accuracy: sessions.length > 0 ? (completedSessions.length / sessions.length) * 100 : 0
    }
  }
